...
The default `--kind` filter for `events wait`/`list` is `board_post,needs_input,permission_request,agent_stop,hook_error`. The other kinds are observability records. Claude also wires `UserPromptSubmit` for prompt delivery confirmation; Codex does not. The adapter is observability-only — it never approves, denies, or changes a permission decision.
