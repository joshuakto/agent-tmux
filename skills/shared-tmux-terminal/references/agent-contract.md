...
`events wait` is idempotent: the prompt mark is your cursor, so re-running with the same mark returns the same event. On timeout, `--json` adds `.events_after_cursor`/`.kinds_after_cursor` to distinguish a stalled worker (0) from one that finished with a different event kind (>0).
