...
Session is inferred from the mark by default; pass `--session` only to override or when no mark is used.
...
Use `--since-mark <mark-id>` after `prompt`; session is inferred from the mark automatically.
...
- `null` (timeout): `events wait --json` emits `{"timeout":true}` on timeout — `jq -r .kind` yields `null`. Read recent output or re-prompt.
