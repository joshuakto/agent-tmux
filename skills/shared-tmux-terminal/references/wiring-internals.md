...
The wrapper exec's `codex -c "$(cat .agent/tmux.d/hooks/<session>/codex-hooks.toml)" ...`. The TOML is a single inline `hooks={Stop=[...],PermissionRequest=[...]}` table. Codex `SessionStart` and `UserPromptSubmit` are not wired — `launch` already emits `session_started`/`session_reused`/`session_recovered`, and Codex prompt submissions have no manager branch.
