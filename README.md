...
Recognized `--run` binaries: first-class native events for Claude Code (`claude --name <name>`) and Codex CLI (`codex`); profile-aware prompt keys for opencode CLI (`opencode`) and Gemini CLI (`gemini`).
